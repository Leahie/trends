...
export default function HomePage() {
  ...
  const { pinnedBoards, addNewRootBoard } = useSidebar();
  ...
  const handleCreateBoard = async () => {
    ...
    if (result) {
      // Add the new board to openBoards in sidebar
      addNewRootBoard(result.id);
    }
    setIsCreating(false);
  };
  ...
  return (
    <div className='w-full h-full overflow-y-auto overflow-x-hidden'>
      <div className='mt-32 mx-16 flex flex-col gap-5 pb-16'>
        <Message firstName={firstName} />
        
        {/* Pinned Boards Section */}
        {pinnedBoardObjects && pinnedBoardObjects.length > 0 && (
          <div className="mb-12">
